...
const CommentarySection: React.FC<CommentarySectionProps> = ({
  ...
  // v1.0.16 — handleViewFullDetails (Cricbuzz external link) removed
  ...
  // v1.0.15 — SCOPE-LIMITED OVER-SEPARATOR.
  // User directive (2026-04-24):
  //   "Banner1 ki 2 placement rakho — ek header, ek sirf LAST 2 OVERS
  //    ke beech ka separator. Puri commentary mein mat rakho. Dono
  //    jagha alag-alag ad maange."
  //
  // Commentary is newest-first (row 0 = most recent ball). So:
  //   1. We walk rows in order and identify the FIRST unique integer
  //      over seen (= the current/most-recent over).
  //   2. The VERY FIRST row whose integer over differs from that is the
  //      boundary between the most-recent over and the over before it
  //      (= the last 2 overs boundary).
  //   3. We render exactly ONE Banner #1 there. No other banners
  //      anywhere else in the commentary scroll.
  //
  // Load delay on this over-separator is 5s so it doesn't race the
  // header Banner #1 (which loads at 0s). Staggered mount times →
  // two independent AdMob auctions → AdMob serves different creatives
  // to the two Banner #1 slots on the same page.
  let firstUniqueOverInt: number | null = null;
  let overSeparatorRendered = false;
  const shouldShowBannerForItem = (item: Commentary, _index: number): boolean => {
    if (overSeparatorRendered) return false;
    ...
    if (firstUniqueOverInt === null) {
      firstUniqueOverInt = overInt;
      return false;
    }
    if (overInt === firstUniqueOverInt) return false;
    // Over changed exactly once — this is the last-2-overs boundary.
    overSeparatorRendered = true;
    return true;
  };

  /** Always slot 0 (Banner #1). Single-fire, so no counter needed. */
  const nextOverBreakSlot = (): number => 0;

  /**
   * Fixed 5s delay for the over-separator Banner #1 — keeps its ad
   * request spaced from the header Banner #1 (which mounts at 0s)
   * so AdMob's auction rotates different creatives.
   */
  const nextOverBreakDelayMs = (): number => 5000;
      ...
      >
        ...
        {displayedCommentary.length === 0 && (
          <View style={{ padding: 20, alignItems: 'center' }}>
            ...
            {/* v1.0.16 — Cricbuzz "View Live Commentary" external link
                button removed per user directive 2026-05-06. */}
          </View>
        )}

        {/* Ball-by-ball commentary */}
        {matchStatus !== 'upcoming' && displayedCommentary.map((item, index) => {
          // v1.0.17 — inline over-break ads inside the commentary stream
          // are DISABLED per user directive (2026-05-17). The over-transition
          // banner injection is short-circuited so no NativeAdCard is rendered
          // between commentary rows. Banner #1 (top header) and Banner #2
          // (relocated under Banner #1 in match/[id].tsx) remain the only
          // visible ads on this screen.
          const showOverBanner = false;
        ...
        {/* v1.0.16 — "View Full Match Details on Web" Cricbuzz button
            removed per user directive 2026-05-06. */}
        <View style={styles.actionContainer}>
          <Text style={styles.infoText}>
            {hasMore
              ? 'Tap "Load More" for older commentary'
              : 'All loaded commentary shown.'}
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  commentaryItem: {
    ...
    paddingVertical: 12,
    paddingHorizontal: 10,
    // v1.0.17 — increased per-row breathing room so each ball card
    // visually separates from neighbours instead of looking crowded.
    // Border-bottom removed in favour of solid margin gap.
    gap: 10,
    borderRadius: 8,
    marginVertical: 5,
    marginHorizontal: 8,
    ...
    // Subtle elevation makes each ball look like its own card.
    elevation: 1,
    shadowColor: '#000',
    shadowOpacity: 0.04,
    shadowRadius: 2,
