...
const SHARE_UNLOCK_DATE_KEY = 'crickapp_share_unlock_date';
const ADS_REQUIRED = 1;
const PRO_DURATION_MS = 30 * 60 * 1000; // 30 minutes in milliseconds

// Local-day key (YYYY-MM-DD) in device local time
...
export const ProProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // v1.0.15 — Daily share-to-unlock quota (1/day).
  // canShareUnlockToday(): true if user hasn't yet claimed today's share unlock.
  // markSharedUnlockToday(): call after a successful native Share.sharedAction.
  const canShareUnlockToday = async (): Promise<boolean> => {
    try {
      const last = await AsyncStorage.getItem(SHARE_UNLOCK_DATE_KEY);
      return last !== todayKey();
    } catch {
      return true; // storage error → do not block the user
    }
  };
